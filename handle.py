'''
MQSH Async Handler, ShellzRus 2016
...
import threading
import time, os, sys, base64, string
...
You can also asynchronously output messages with Commander.output('message') """
    ...
    PALLETE=[('reversed', urwid.BLACK, urwid.LIGHT_GRAY),
              ('altreversed', urwid.WHITE, urwid.DARK_BLUE),
              ('normal', urwid.LIGHT_GRAY, urwid.BLACK),
              ('error', urwid.LIGHT_RED, urwid.BLACK),
              ('green', urwid.LIGHT_GREEN, urwid.BLACK),
              ('yellow',urwid.YELLOW, urwid.BLACK),
              ('blue', urwid.LIGHT_BLUE, urwid.BLACK),
              ('magenta', urwid.DARK_MAGENTA, urwid.BLACK), ]
    
    
    def __init__(self, title, command_caption='Commands: publish <message>, echo <message>, [tab] to switch window, [up/down/pageup/pagedown] to scroll ', cmd_cb=None, max_size=1000):
        ...
        foot=urwid.Pile([urwid.AttrMap(urwid.Text(command_caption), 'altreversed'),
                        urwid.AttrMap(self.input,'normal')])
        urwid.Frame.__init__(self, 
                             urwid.AttrWrap(self.body, 'normal'),
                             urwid.AttrWrap(self.header, 'altreversed'),
                             foot)
    ...
    def on_line_entered(self,line):
        if self._cmd:
            ...
            if line in ('q','quit','exit'):
                os.system("kill `cat /tmp/sub.pid`;rm /tmp/sub.pid")
                raise urwid.ExitMainLoop()
            else:
                self.output(line)
...
if __name__=='__main__':
    ...
    class TestCmd(Command):
        ...
        def do_publish(self, *args):
             ...
             args = str(args)
             enctext = base64.b64encode(args)
             os.system('mosquitto_pub -h 127.0.0.1 -p 1883 -t shell/#  -m "%s"' % enctext)

    c=Commander('MQSH Handler, Async with Python', cmd_cb=TestCmd())
   ...
   # def file_size_change():
    ...
    def run():
         os.system("if ([ ! -f /tmp/sub.pid ]);then mosquitto_sub -h 127.0.0.1 -t data/# >return & echo $! > sub.pid;fi")
        while True:
            fsize = os.stat('return').st_size
            try:
                fsize_current =  os.stat('return').st_size
                if fsize != fsize_current:
                    fret = open('return')
                    fcmdret = fret.read()
                    #if fcmdret:
                    c.output(fcmdret, 'yellow')
            except:
                pass
            #time.sleep(1)
            #if is_non_zero_file('return'):
                #ret = open('return', 'rw+')
                #cmdret = ret.read()
                #c.output(cmdret, 'green')
            #ret.truncate()
            #ret.close
    t=Thread(target=run)
    t.daemon=True
